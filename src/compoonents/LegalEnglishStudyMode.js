...
function LegalEnglishStudyMode(props) {
  const [nextWord, setNextWord] = useState(0);
  const [count, setCount] = useState(1);
  //console.log(vocabulary[0])
  const next = (event) => {
    event.preventDefault();
    if(count <= props.legalVocabulary.legal_english.length) {
  ...
  return (
    ...
    <div className="center">
      ...
      <p>Legal English vocabulary - Study Mode</p>
        <div className="card-header">
          <p className='en-word'>{props.legalVocabulary.legal_english[nextWord].english_word}</p>
        </div>
        <div className="card-body">
        <p>{props.legalVocabulary.legal_english[nextWord].translation}</p>
        </div>
...
export default LegalEnglishStudyMode;